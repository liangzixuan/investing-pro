...
describe("PersonalFinancialScreener", () => {
  ...
  it.each(["commonDividendsPaid", "commonStockRepurchases"] as const)(
    ...
    async (field) => {
      ...
      for (const value of ["-125.5", "0", "12.50001"]) {
        const result = response();
        const cell = paymentCell(field, value);
        api.screenPersonalFinancials.mockResolvedValueOnce({
          ...result,
          rows: result.rows.map((row) => ({
            ...row,
        ...
        expect(text(inspector(render()))).toContain("2024-09-29");
        expect(text(inspector(render()))).toContain("2025-09-27");
        expect(text(inspector(render()))).toContain("0000000001-26-000091");
        expect(text(inspector(render()))).toContain(
  ...
  it.each(["investingCashFlow", "financingCashFlow"] as const)(
    "displays signed %s values and exact independent annual evidence, including unknowns",
    async (field) => {
      const label =
        field === "investingCashFlow"
          ? "Reported investing cash flow (USD)"
          : "Reported financing cash flow (USD)";
      for (const value of ["-125.5", "0", "12.50001"]) {
        const result = response();
        const cell = activityCell(field, value);
        api.screenPersonalFinancials.mockResolvedValueOnce({
          ...result,
          rows: result.rows.map((row) => ({
            ...row,
        ...
        expect(text(inspector(render()))).toContain("0000000001-26-000091");
        expect(text(inspector(render()))).toContain(
          "A positive amount is not inherently favorable",
        );
        harness.unmount();
        harness.reset();
      }
  ...
  it("preserves a literal old twenty-four-column saved view and explicitly saves common payment criteria and columns", async () => {
    ...
    const old = {
      ...savedViewFixture("old-twenty-four"),
      display: { visibleMetrics: oldMetrics },
    };
    const untouched = { ...savedViewFixture("legacy"), display: null };
    api.fetchPersonalFinancialSavedViews.mockResolvedValueOnce({
    ...
    submit(render());
    await flush();
    expect(columnHeaders(render())).toHaveLength(26);
    expect(columnHeaders(render())).not.toContain(
      "Reported common dividends paid (USD)",
    );
    expect(columnHeaders(render())).not.toContain(
    ...
    change(render(), "Financial column view", "commonStockPayments");
    click(render(), "Add financial filter");
    change(render(), "Financial metric 1", "commonDividendsPaid");
    change(render(), "Financial threshold 1", "0");
    change(render(), "Financial sort field", "commonStockRepurchases");
    submit(render());
    await flush();
    ...
    expect(payload.views[0]!.criteria.clauses).toEqual([
      { field: "commonDividendsPaid", operator: "gte", value: "0" },
    ]);
    expect(payload.views[0]!.criteria.sort.field).toBe(
      "commonStockRepurchases",
    );
    expect(payload.views[1]).toEqual(untouched);
    harness.unmount();
    ...
    expect(text(inspector(render()))).toContain("Exact value: -125.5 USD");
    expect(text(inspector(render()))).toContain(
      "ordinary cash dividends paid to the parent",
    );
    inspectCell(
      comparisonPanel(render()),
      "Reported common stock repurchase payments (USD)",
    ...
    expect(text(inspector(render()))).toContain("Exact value: 0 USD");
    expect(text(inspector(render()))).toContain(
      "The two payments do not sum to net financing cash flow",
    );
  });

  it("preserves a literal old twenty-two-column saved view and explicitly saves investing/financing criteria and columns", async () => {
    const oldMetrics = [
      "revenue",
      "grossProfit",
      "netIncome",
    ...
    const old = {
      ...savedViewFixture("old-twenty-two"),
      display: { visibleMetrics: oldMetrics },
    };
    const untouched = { ...savedViewFixture("legacy"), display: null };
    api.fetchPersonalFinancialSavedViews.mockResolvedValueOnce({
    ...
    submit(render());
    await flush();
    expect(columnHeaders(render())).toHaveLength(24);
    expect(columnHeaders(render())).not.toContain(
      "Reported investing cash flow (USD)",
    );
    expect(columnHeaders(render())).not.toContain(
    ...
    change(render(), "Financial column view", "cashFlow");
    click(render(), "Add financial filter");
    change(render(), "Financial metric 1", "investingCashFlow");
    change(render(), "Financial threshold 1", "0");
    change(render(), "Financial sort field", "financingCashFlow");
    submit(render());
    await flush();
    click(render(), "Save financial view");
    await flush();
    ...
    expect(payload.views[0]!.criteria.clauses).toEqual([
      { field: "investingCashFlow", operator: "gte", value: "0" },
    ]);
    expect(payload.views[0]!.criteria.sort.field).toBe("financingCashFlow");
    expect(payload.views[1]).toEqual(untouched);
    harness.unmount();
    harness.reset();
    api.fetchPersonalFinancialSavedViews.mockResolvedValueOnce({
  ...
  it("preserves a literal old twenty-column saved view and explicitly saves cash/equity criteria and columns", async () => {
    const oldMetrics = [
      "revenue",
      "grossProfit",
      "netIncome",
    ...
    submit(render());
    await flush();
    expect(columnHeaders(render())).toHaveLength(22);
    expect(columnHeaders(render())).not.toContain(
      "Reported cash and cash equivalents (USD)",
    );
    expect(columnHeaders(render())).not.toContain(
      "Reported stockholders' equity (USD)",
    );
    change(render(), "Financial column view", "q4Balances");
    click(render(), "Add financial filter");
    change(render(), "Financial metric 1", "cashAndCashEquivalents");
    change(render(), "Financial threshold 1", "0");
    change(render(), "Financial sort field", "stockholdersEquity");
    submit(render());
    await flush();
    click(render(), "Save financial view");
    await flush();
    ...
    expect(payload.views[0]!.criteria.clauses).toEqual([
      { field: "cashAndCashEquivalents", operator: "gte", value: "0" },
    ]);
    expect(payload.views[0]!.criteria.sort.field).toBe("stockholdersEquity");
    expect(payload.views[1]).toEqual(untouched);
    harness.unmount();
    harness.reset();
    api.fetchPersonalFinancialSavedViews.mockResolvedValueOnce({
    ...
    expect(text(inspector(render()))).toContain("Exact value: 0 USD");
    expect(text(inspector(render()))).toContain(
      "Broader cash-and-restricted-cash concepts are not substituted",
    );
    inspectCell(
      comparisonPanel(render()),
      "Reported stockholders' equity (USD)",
    ...
    expect(text(inspector(render()))).toContain("Exact value: -12.5 USD");
    expect(text(inspector(render()))).toContain(
      "excluding temporary equity and noncontrolling interests",
    );
  });
  ...
  it.each([
    ...
    "explains each unknown %s without inferring an amount",
    async (field, concept, label) => {
      for (const reason of [
        ...
        const sources =
          reason === "conflicting" || reason === "unsupported_balance_date"
            ? [
                {
                  concept,
                  asOfDate: date,
        ...
        expect(text(inspector(render()))).not.toContain("reported total is");
        if (sources.length)
          expect(text(inspector(render()))).toContain(
  ...
  it("reuses all24 columns, comparison and source inspection for selected saved companies", async () => {
    configureWatchlist(3);
    api.screenPersonalFinancials.mockImplementation(
      (request: PersonalFinancialScreenRequestDto) =>
        Promise.resolve(watchlistResponse(request)),
...
function defaultInstantCells() {
  ...
  return {
    ...
    cashAndCashEquivalents: operand(
      "CashAndCashEquivalentsAtCarryingValue",
      "0",
    ),
    stockholdersEquity: operand("StockholdersEquity", "-12.5"),
  };
}
...
function response(offset = 0, count = 1): PersonalFinancialScreenResponseDto {
  return {
    ...
    sources: [
      ...
      concept,
      status: "available",
      sourceUrl: `https://data.sec.gov/api/xbrl/frames/us-gaap/${concept}/USD/CY2025${concept === "AssetsCurrent" || concept === "LiabilitiesCurrent" || concept === "Assets" || concept === "Liabilities" || concept === "CashAndCashEquivalentsAtCarryingValue" || concept === "StockholdersEquity" ? "Q4I" : ""}.json`,
    })),
    rows: Array.from(
...
function paymentCell(
  field: "commonDividendsPaid" | "commonStockRepurchases",
  value: string,
): PersonalFinancialScreenAnnualCellDto {
  return {
    status: "available",
    ...
    sources: [
      {
        ...
        accessionNumber: "0000000001-26-000091",
        startDate: "2024-09-29",
        endDate: "2025-09-27",
        value,
      },
...
function activityCell(
  field: "investingCashFlow" | "financingCashFlow",
  value: string,
): PersonalFinancialScreenAnnualCellDto {
  return {
    status: "available",
