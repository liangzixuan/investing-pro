...
export const PERSONAL_FILING_SELECTED_FACTS_PATH =
  "/v1/personal-filing/selected-facts" as const;
...
export async function registerPersonalSelectedFactRoute(
  app: FastifyInstance,
  ...
  if (
    ...
    !isPersonalSelectedFactReleaseCapability(capability)
  ) {
  ...
  await app.register((routes, _options, done) => {
    routes.get(
      ...
      (request, reply) => {
        if (!isAllowedRequest(request, listenOptions))
        ...
        return reply
          ...
          .send(getPersonalSelectedFactReleaseResponse(capability));
      },
    );
    done();
  });
...
function isAllowedRequest(
  ...
  if (request.url !== PERSONAL_FILING_SELECTED_FACTS_PATH) return false;
...
function sendUnavailableProblem(reply: FastifyReply, request: FastifyRequest) {
  return sendProblem(reply, request, 404, "Selected facts unavailable");
}
...
function sendProblem(
  ...
  const problem: ProblemDetailsDto = {
    ...
    detail:
      status === 403
        ? "The local selected-fact request was not accepted."
        : "No personal selected-fact release is available.",
    instance: PERSONAL_FILING_SELECTED_FACTS_PATH,
    traceId: request.id,
  };
  return reply
    .status(status)
