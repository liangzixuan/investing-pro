  ...
  it("preserves full-dollar precision, computes shared margin definitions, and exposes source durations", () => {
    ...
    const result = run(
      ...
      snapshot({
        ...
        Revenues: [fact(1, "300000000000000000000")],
      ...
      const result = run(
        ...
        replaceFrame(
          snapshot({
            ...
            NetCashProvidedByUsedInInvestingActivities: [fact(1, "-15")],
            NetCashProvidedByUsedInFinancingActivities: [fact(1, "-5")],
            PaymentsOfDividendsCommonStock: [fact(1, "2")],
            PaymentsForRepurchaseOfCommonStock: [fact(1, "3")],
            InterestPaidNet: [fact(1, "4")],
            IncomeTaxesPaidNet: [fact(1, "6")],
...
describe("PP&E purchases and operating cash flow less PP&E purchases", () => {
  ...
  it("versions only the screen formula set and keeps existing ratio metadata", () => {
    expect(PERSONAL_FINANCIAL_SCREEN_FORMULA_SET_VERSION).toBe("1.8.0");
...
describe("operating cash flow / net income", () => {
  ...
  it("registers a distinct formula while preserving the eleven metrics and every prior formula version", () => {
    expect(PERSONAL_FINANCIAL_SCREEN_ANNUAL_METRICS.at(17)).toBe(metric);
    expect(PERSONAL_FINANCIAL_SCREEN_ANNUAL_METRICS).toHaveLength(19);
    expect(PERSONAL_SEC_ANNUAL_CONCEPTS).toHaveLength(14);
  ...
  it("preserves ten compatible cells while net margin and cash conversion withhold mixed filings", () => {
    const input = snapshot({
      [REVENUE]: [fact(1, "100")],
      GrossProfit: [fact(1, "40")],
    ...
    expect(Object.keys(expected)).toEqual(
      PERSONAL_FINANCIAL_SCREEN_ANNUAL_METRICS.filter(
        (field) =>
          field !== "investingCashFlow" &&
          field !== "financingCashFlow" &&
          field !== "commonDividendsPaid" &&
          field !== "commonStockRepurchases" &&
    ...
    for (const oldMetric of Object.keys(
      ...
      expect(result.rows[0]!.metrics[oldMetric]).toEqual(
        oldMetric === "netMargin"
          ? { status: "unavailable", reason: "filing_mismatch", unit, sources }
          : { status: "available", value, unit, sources },
      );
...
describe("operating cash flow less PP&E purchases / selected revenue", () => {
  ...
  it("retains the annual formula in the expanded twenty-one-Frame transport and saved criteria grammar", () => {
...
describe("consistent filings for original three margins", () => {
  const margins = [
  ...
  const otherFiling = "0000000001-26-000002";

  for (const [metric, numerator, concept] of margins) {
    describe(metric, () => {
      function cells(
        input: PersonalSecFinancialSnapshotDto,
        revenueBasis: PersonalFinancialRevenueBasisDto = "agreement",
      ) {
        return run([identity("FILING", 1)], input, {
          ...criteria(),
          revenueBasis,
        }).rows[0]!.metrics;
      }
      function input(value = "25") {
        return snapshot({
          [REVENUE]: [fact(1, "100")],
          Revenues: [fact(1, "100.0")],
          SalesRevenueNet: [fact(1, "100.00")],
          [concept]: [fact(1, value)],
        });
      }

      it.each(PERSONAL_FINANCIAL_REVENUE_BASES)(
        "preserves compatible %s arithmetic and withholds cross-operand filings without changing reported cells",
        (revenueBasis) => {
          const source = input();
          const compatible = cells(source, revenueBasis);
          expect(compatible[metric]).toEqual({
            status: "available",
            unit: "percent",
            value: "25.00",
            sources: [
              ...compatible[numerator].sources,
              ...compatible.revenue.sources,
            ],
          });
          const mismatch = cells(
            replaceFrame(source, concept, {
              facts: [fact(1, "25", { accessionNumber: otherFiling })],
            }),
            revenueBasis,
          );
          expect(mismatch.revenue).toEqual(compatible.revenue);
          expect(mismatch[numerator]).toMatchObject({
            status: "available",
            unit: "USD",
            value: "25",
          });
          expect(mismatch[metric]).toEqual({
            status: "unavailable",
            unit: "percent",
            reason: "filing_mismatch",
            sources: [
              ...mismatch[numerator].sources,
              ...mismatch.revenue.sources,
            ],
          });
          expect(mismatch[metric].sources[0]).toEqual({
            concept,
            accessionNumber: otherFiling,
            startDate: "2025-01-01",
            endDate: "2025-12-31",
            value: "25",
          });
      ...
      for (const location of ["numerator", "revenue"] as const) {
        it.each(PERSONAL_FINANCIAL_REVENUE_BASES)(
          `checks non-first ${location} references under %s and retains original strings, order and multiplicity`,
          (revenueBasis) => {
            const selectedConcept =
              location === "numerator"
                ...
                : revenueBasis === "agreement"
                  ? REVENUE
                  : revenueBasis;
            const amount = location === "numerator" ? "25" : "100";
            // Reverse provider input order deliberately. The existing engine's
            // sorted source order is retained, including equivalent duplicates.
            const source = replaceFrame(input(), selectedConcept, {
              facts: [
                fact(1, `${amount}.00`, { accessionNumber: otherFiling }),
                fact(1, amount),
                fact(1, amount),
              ],
            });
            const before = JSON.stringify(source);
            const result = cells(source, revenueBasis);
            expect(result[numerator].status).toBe("available");
            expect(result.revenue.status).toBe("available");
            const changedOperand =
              location === "numerator" ? result[numerator] : result.revenue;
            expect(
              changedOperand.sources
                .filter((reference) => reference.concept === selectedConcept)
                .map((reference) => [
                  reference.accessionNumber,
                  reference.value,
                ]),
            ).toEqual([
              ["0000000001-26-000001", amount],
              ["0000000001-26-000001", amount],
              [otherFiling, `${amount}.00`],
            ]);
            expect(result[metric]).toEqual({
              status: "unavailable",
              reason: "filing_mismatch",
      ...
      it("checks the last agreeing revenue concept while explicit unaffected bases remain usable", () => {
        const source = replaceFrame(input(), "SalesRevenueNet", {
          facts: [fact(1, "100.00", { accessionNumber: otherFiling })],
        });
        ...
        expect(
          result[metric].sources.map((reference) => reference.concept),
        ).toEqual([concept, REVENUE, "Revenues", "SalesRevenueNet"]);
        for (const revenueBasis of [REVENUE, "Revenues"] as const)
          expect(cells(source, revenueBasis)[metric]).toMatchObject({
            status: "available",
            value: "25.00",
          });
      });

      it("keeps revenue, numerator, period, filing and domain precedence for simultaneous defects", () => {
        const source = snapshot({
          [REVENUE]: [fact(1, "0")],
          [concept]: [
            fact(1, "25", {
        ...
        expect(
          cells(replaceFrame(source, concept, { facts: [fact(1, "25")] }))[
            metric
          ],
        ).toMatchObject({ reason: "nonpositive_revenue" });
        expect(
          cells(replaceFrame(source, concept, { facts: [] }))[metric],
        ).toMatchObject({ reason: "missing" });
        expect(
          cells(replaceFrame(source, concept, { unknownCiks: [cik(1)] }))[
            metric
          ],
        ).toMatchObject({ reason: "conflicting" });
        const invalidNumerator = replaceFrame(source, concept, {
          facts: [fact(1, "NaN")],
        });
        expect(cells(invalidNumerator)[metric]).toMatchObject({
          reason: "invalid_value",
        });
        expect(
          cells(replaceFrame(invalidNumerator, REVENUE, { facts: [] }))[metric],
        ).toMatchObject({ reason: "missing" });
        const failedNumerator = replaceFrame(source, concept, {
          status: "upstream_unavailable",
          facts: [],
        });
        expect(
          cells(
            replaceFrame(failedNumerator, REVENUE, {
              facts: [fact(1, "0"), fact(1, "1")],
            }),
          )[metric],
        ).toMatchObject({ reason: "conflicting" });
        expect(
      ...
      it("preserves shared-CIK coverage, three-valued filtering and stable paging", () => {
        const companies = [
          identity("MIXED", 1),
          { ...identity("MIXED.B", 1), listingId: "listing-0000000001-b" },
          identity("COMPATIBLE", 2),
        ];
        const source = snapshot({
          [REVENUE]: [fact(1, "100"), fact(2, "100")],
          [concept]: [
            fact(1, "25", { accessionNumber: otherFiling }),
            fact(2, "25"),
          ],
        });
        const full = run(companies, source);
        expect(full.rows[1]!.metrics[metric]).toEqual(
          full.rows[2]!.metrics[metric],
        );
        expect(full.metricCoverage[metric]).toEqual({ known: 1, unknown: 2 });
        const filtered = run(
          companies,
          source,
          criteria([clause(metric, "gte", "25")]),
        );
        expect(filtered).toMatchObject({
          totalMatches: 1,
          totalUnknown: 2,
        ...
        expect(
          run(
            ...
            source,
            criteria([
              clause(metric, "gte", "25"),
              clause("revenue", "lte", "99"),
            ]),
          ),
        ).toMatchObject({
          totalMatches: 0,
        ...
        const pages = [0, 1, 2].map((offset) =>
          run([...companies].reverse(), source, criteria(), {
            offset,
            limit: 1,
          }),
        );
        ...
        expect(
          pages.every(
            (page) =>
              page.metricCoverage[metric].known === 1 &&
              page.metricCoverage[metric].unknown === 2,
          ),
        ).toBe(true);
      });
    });
  ...
  it.each([
    ...
    ["-0.000001", "100", "0.00"],
    ["250", "100", "250.00"],
    ["100000000000000000000", "300000000000000000000", "33.33"],
  ])(
    "retains all three margins' exact compatible %s / %s arithmetic as %s",
    (amount, revenue, expected) => {
      const result = run(
        [identity("PRECISE", 1)],
        snapshot({
          [REVENUE]: [fact(1, revenue)],
          NetIncomeLoss: [fact(1, amount)],
          OperatingIncomeLoss: [fact(1, amount)],
          NetCashProvidedByUsedInOperatingActivities: [fact(1, amount)],
        }),
      );
...
describe("reported investing and financing cash flows", () => {
  const reported = [
  ...
  it("adds two exact annual fields while retaining the old twenty-two order and formula set", () => {
    expect(
    ...
    expect(PERSONAL_FINANCIAL_SCREEN_METRICS.slice(4, 7)).toEqual([
      "operatingCashFlow",
      "investingCashFlow",
      "financingCashFlow",
    ]);
    const result = run([identity("ACTIVITIES", 1)], snapshot());
    expect(result).toMatchObject({
      schemaVersion: "14.0.0",
      formulaVersion: "1.8.0",
    });
    expect(result.sources.length + result.priorRevenueSources.length).toBe(23);
    for (const [metric, concept] of reported) {
  ...
  describe.each(reported)("%s from %s only", (metric, concept) => {
    const otherMetric =
      metric === "investingCashFlow"
        ? "financingCashFlow"
        : "investingCashFlow";
    const otherConcept =
      concept === "NetCashProvidedByUsedInInvestingActivities"
        ? "NetCashProvidedByUsedInFinancingActivities"
        : "NetCashProvidedByUsedInInvestingActivities";
    const input = (patch: Partial<PersonalSecAnnualFrameDto>) =>
      replaceFrame(
        snapshot({
          [concept]: [fact(1, "100")],
          [otherConcept]: [
    ...
    it.each([
      ...
      "preserves exact reported signed amount %s and its own source text",
      (value, canonical) => {
        const source = fact(1, value, {
          startDate: "2024-12-29",
          endDate: "2025-12-27",
        });
        const cells = run([identity("ACTIVITY", 1)], input({ facts: [source] }))
          .rows[0]!.metrics;
        ...
        expect(cells[otherMetric]).toMatchObject({
          status: "available",
          value: "-50",
          sources: [
            {
        ...
        expect(cells.operatingCashFlow).toMatchObject({ value: "30" });
        expect(cells.operatingCashFlowLessPpePurchases).toMatchObject({
          value: "25",
        });
      },
    );
    ...
    it.each([
      ...
      [{ facts: [fact(1, "100"), fact(1, "101")] }, "conflicting"],
      [
        { facts: [fact(1, "100"), fact(1, "100", { endDate: "2025-12-30" })] },
        "conflicting",
      ],
      [
        {
          facts: [fact(1, "100"), fact(1, "100", { startDate: "2025-01-02" })],
        },
        "conflicting",
      ],
      [{ facts: [fact(1, "NaN")] }, "invalid_value"],
    ] satisfies readonly (readonly [
      Partial<PersonalSecAnnualFrameDto>,
      string,
    ])[])("isolates unavailable source %j as %s", (patch, reason) => {
      const source = input(patch);
      const result = run([identity("ACTIVITY", 1)], source);
      expect(result.rows[0]!.metrics[metric]).toMatchObject({
        status: "unavailable",
        unit: "USD",
        reason,
      });
      expect(result.rows[0]!.metrics[otherMetric]).toMatchObject({
        status: "available",
        value: "-50",
      });
      expect(
        result.rows[0]!.metrics.operatingCashFlowLessPpePurchases,
      ...
      ).toMatchObject({ totalMatches: 0, totalUnknown: 1 });
    });

    it("retains agreeing duplicate facts and their independent accessions", () => {
      const cells = run(
        [identity("ACTIVITY", 1)],
        input({
          facts: [
            fact(1, "-125.50"),
            fact(1, "-125.5", { accessionNumber: "0000000001-26-000002" }),
          ],
        }),
      ).rows[0]!.metrics;
      ...
      expect(cells[metric].sources).toHaveLength(2);
    });

    it.each([
      {
        sourceUrl: `https://data.sec.gov/api/xbrl/frames/us-gaap/${concept}/EUR/CY2025.json`,
      },
      {
        sourceUrl: `https://data.sec.gov/api/xbrl/frames/us-gaap/${concept}/USD/CY2025Q4I.json`,
      },
      ...
      { facts: [fact(1, "1", { startDate: "2025-02-30" })] },
      { facts: [fact(1, "1", { startDate: "2026-01-01" })] },
    ])(
      "rejects a substituted or structurally invalid direct source %j",
      (patch) => {
        expect(() =>
          run([], input(patch as Partial<PersonalSecAnnualFrameDto>)),
        ).toThrow("Personal financial screen request is invalid.");
      },
    );

    it("filters exact signed amounts, retains zero and duplicate listings, and sorts unknowns last", () => {
      const first = identity("NEGATIVE", 1);
      const companies = [
        first,
        { ...first, symbol: "NEGATIVE.B", listingId: "class-b" },
        identity("ZERO", 2),
        identity("POSITIVE", 3),
        identity("MISSING", 4),
      ];
      const source = input({
        facts: [
          fact(1, "-9007199254740993.0001"),
          fact(2, "0"),
          fact(3, "9007199254740993.0002"),
        ],
      });
      const selected = {
        ...criteria([clause(metric, "gte", "-9007199254740993.0000")]),
        sort: { field: metric, direction: "asc" as const },
      };
      expect(validatePersonalFinancialScreenCriteria(selected)).toBe(true);
      const result = run(companies, source, selected);
      ...
      expect(result.metricCoverage[metric]).toEqual({ known: 4, unknown: 1 });
      expect(
        run(
          companies,
          source,
          criteria([clause(metric, "lte", "-9007199254740993.0001")]),
        ).rows.map((row) => row.identity.symbol),
      ).toEqual(["NEGATIVE", "NEGATIVE.B"]);
      for (const direction of ["asc", "desc"] as const) {
        const sorted = run(companies, source, {
          ...criteria(),
          sort: { field: metric, direction },
        });
        expect(sorted.rows.at(-1)!.identity.symbol).toBe("MISSING");
        expect(sorted.rows[0]!.identity.symbol).toBe(
          direction === "asc" ? "NEGATIVE" : "POSITIVE",
        );
      }
    });
  });

  it("preserves all twenty-two old cells and coverage under independent new source states across every revenue basis", () => {
    let source = withPriorRevenue(
      snapshot(
        Object.fromEntries(
    ...
    source = {
      ...
      instantFrames: source.instantFrames.map((frame) => ({
        ...frame,
        facts: [instantFact(1, "50")],
      })),
    };
    const companies = [identity("OLD", 1), identity("ABSENT", 2)];
    for (const revenueBasis of PERSONAL_FINANCIAL_REVENUE_BASES) {
      const selected = { ...criteria(), revenueBasis };
      const before = run(companies, source, selected);
      for (const [newMetric, concept] of reported)
        for (const patch of [
          { facts: [fact(1, "-999.125")] },
          { facts: [] },
          { facts: [], status: "upstream_unavailable" },
          { unknownCiks: [cik(1)] },
        ] satisfies Partial<PersonalSecAnnualFrameDto>[]) {
          const after = run(
            companies,
            replaceFrame(source, concept, patch),
            selected,
          );
          for (let row = 0; row < before.rows.length; row++)
            for (const metric of PERSONAL_FINANCIAL_SCREEN_METRICS.filter(
              ...
              expect(after.rows[row]!.metrics[metric]).toEqual(
                before.rows[row]!.metrics[metric],
              );
...
describe("reported common dividends and common share repurchases", () => {
  const reported = [
  ...
  it("adds two exact annual fields while retaining the old twenty-four order and formula set", () => {
    expect(
      PERSONAL_FINANCIAL_SCREEN_METRICS.filter(
        (metric) =>
          metric !== "commonDividendsPaid" &&
    ...
    expect(PERSONAL_FINANCIAL_SCREEN_METRICS.slice(6, 9)).toEqual([
      "financingCashFlow",
      "commonDividendsPaid",
      "commonStockRepurchases",
    ]);
    const result = run([identity("PAYMENTS", 1)], snapshot());
    expect(result).toMatchObject({
      schemaVersion: "14.0.0",
      formulaVersion: "1.8.0",
    });
  ...
  describe.each(reported)("%s from %s only", (metric, concept) => {
    ...
    const input = (patch: Partial<PersonalSecAnnualFrameDto>) =>
      replaceFrame(
        snapshot({
          ...
          NetCashProvidedByUsedInOperatingActivities: [fact(1, "30")],
          NetCashProvidedByUsedInFinancingActivities: [fact(1, "-15")],
          PaymentsToAcquirePropertyPlantAndEquipment: [fact(1, "5")],
        }),
        concept,
        patch,
    ...
    it.each([
      ...
      (value, canonical) => {
        ...
        const cells = run([identity("PAYMENT", 1)], input({ facts: [source] }))
          .rows[0]!.metrics;
        expect(cells[metric]).toEqual({
          status: "available",
          unit: "USD",
        ...
        expect(cells.operatingCashFlow).toMatchObject({ value: "30" });
        expect(cells.financingCashFlow).toMatchObject({ value: "-15" });
        expect(cells.operatingCashFlowLessPpePurchases).toMatchObject({
          value: "25",
        });
      },
    ...
    it.each([
      ...
      const source = input(patch);
      const result = run([identity("PAYMENT", 1)], source);
      expect(result.rows[0]!.metrics[metric]).toMatchObject({
        status: "unavailable",
        unit: "USD",
        reason,
    ...
    it.each([
      ...
      { concept: "PaymentsOfDividendsPreferredStockAndPreferenceStock" },
      { concept: "PaymentsForRepurchaseOfEquity" },
      { concept: "PaymentsForRepurchaseOfPreferredStockAndPreferenceStock" },
      { concept: otherConcept },
      { concept: "NetCashProvidedByUsedInOperatingActivities" },
      { concept: "PaymentsToAcquirePropertyPlantAndEquipment" },
      { facts: [fact(1, "1", { startDate: "2025-02-30" })] },
  ...
  it("preserves all twenty-four old cells and coverage under independent new source states across every revenue basis", () => {
    let source = withPriorRevenue(
      snapshot(
        Object.fromEntries(
          CONCEPTS.map((concept) => [concept, [fact(1, "100")]]),
...
describe("reported interest and income-tax cash payments", () => {
  const reported = [
  ...
  it("adds two exact annual fields while retaining the old twenty-six order and formula set", () => {
    expect(
      PERSONAL_FINANCIAL_SCREEN_METRICS.filter(
        (metric) =>
    ...
    expect(PERSONAL_FINANCIAL_SCREEN_METRICS.slice(8, 11)).toEqual([
      "commonStockRepurchases",
      "interestPaidNet",
      "incomeTaxesPaidNet",
    ]);
  ...
  describe.each(reported)("%s from %s only", (metric, concept) => {
    const otherMetric =
      metric === "interestPaidNet" ? "incomeTaxesPaidNet" : "interestPaidNet";
    const otherConcept =
      concept === "InterestPaidNet" ? "IncomeTaxesPaidNet" : "InterestPaidNet";
    const input = (patch: Partial<PersonalSecAnnualFrameDto>) =>
      replaceFrame(
        snapshot({
          [concept]: [fact(1, "100")],
    ...
    it.each([
      ...
      { concept: "InterestPaidCapitalized" },
      { concept: "InterestExpense" },
      { concept: "IncomeTaxesPaid" },
      { concept: "IncomeTaxExpenseBenefit" },
      { concept: "IncomeTaxRefundsDiscontinuedOperations" },
      { concept: otherConcept },
      { concept: "NetCashProvidedByUsedInOperatingActivities" },
      { concept: "PaymentsToAcquirePropertyPlantAndEquipment" },
      { facts: [fact(1, "1", { startDate: "2025-02-30" })] },
  ...
  it("preserves all twenty-six old cells and coverage under independent new source states across every revenue basis", () => {
    let source = withPriorRevenue(
      snapshot(
        Object.fromEntries(
          CONCEPTS.map((concept) => [concept, [fact(1, "100")]]),
...
describe("reported total assets and total liabilities", () => {
  ...
  it("adds only two direct instant amounts, retaining the old relative order and formula set", () => {
    ...
    expect(PERSONAL_FINANCIAL_SCREEN_INSTANT_METRICS.slice(-4, -2)).toEqual([
      "totalAssets",
      "totalLiabilities",
    ]);
    expect(result).toMatchObject({
...
describe("reported cash and stockholders equity", () => {
  const reported = [
  ...
  function replaceReported(
    input: PersonalSecFinancialSnapshotDto,
    concept: "CashAndCashEquivalentsAtCarryingValue" | "StockholdersEquity",
    patch: Partial<PersonalSecInstantFrameDto>,
  ): PersonalSecFinancialSnapshotDto {
    return {
      ...input,
  ...
  describe.each(reported)("%s from %s only", (metric, concept) => {
    ...
    function input(patch: Partial<PersonalSecInstantFrameDto>) {
      return replaceReported(
        replaceReported(
          {
            ...balances([instantFact(1, "25")], [instantFact(1, "10")]),
            instantFrames: balances(
              [instantFact(1, "25")],
              [instantFact(1, "10")],
            ).instantFrames.map((frame) =>
              frame.concept === "Assets" || frame.concept === "Liabilities"
                ? {
                    ...frame,
                    facts: [
                      instantFact(
                        1,
                        frame.concept === "Assets" ? "1000" : "750",
                      ),
                    ],
                  }
                : frame,
    ...
    it.each(["2025-10-01", "2025-11-29", "2025-12-31"])(
      "retains its own valid date %s without requiring the other amount's date or accession",
      (asOfDate) => {
        const result = cells(
          input({
            facts: [
    ...
    it.each(["2024-12-31", "2025-09-30", "2026-01-01"])(
      "retains unsupported date %s as evidence without changing the valid other amount",
      (asOfDate) => {
        const result = cells(
          input({ facts: [instantFact(1, "100", { asOfDate })] }),
        );
    ...
    it.each([
      ...
      "keeps direct concept state %j unavailable as %s without deriving cash or equity from other balances",
      (patch, reason) => {
        const source = input(patch);
        const result = cells(source);
        expect(result[metric]).toMatchObject({
        ...
        expect(result.totalAssets).toMatchObject({
          status: "available",
          value: "1000",
        });
        expect(result.totalLiabilities).toMatchObject({
          status: "available",
          value: "750",
        });
        const filtered = run(
    ...
    it("retains agreeing duplicate facts and every accession for a direct amount", () => {
      const result = cells(
        input({
          facts: [
            instantFact(1, "100.00"),
  ...
  it("preserves all twenty old values, evidence and unknown reasons across cash and equity source states", () => {
    const annual = withPriorRevenue(
      snapshot(
        Object.fromEntries(
          CONCEPTS.map((concept) => [concept, [fact(1, "100")]]),
      ...
      instantFrames: balances(
        ...
        frame.concept === "Assets" || frame.concept === "Liabilities"
          ? {
              ...frame,
              facts: [
                instantFact(1, frame.concept === "Assets" ? "1000" : "750"),
              ],
            }
          : frame,
      ),
    ...
    for (const revenueBasis of PERSONAL_FINANCIAL_REVENUE_BASES) {
      for (const source of [
        ...
        const selected = { ...criteria(), revenueBasis };
        const before = run([identity("OLD", 1)], source, selected);
        for (const [, concept] of reported) {
          for (const patch of patches) {
            const after = run(
              [identity("OLD", 1)],
              replaceReported(source, concept, patch),
              selected,
            );
            for (const metric of oldMetrics) {
              expect(after.rows[0]!.metrics[metric]).toEqual(
...
describe("fixed Q4 current assets / current liabilities", () => {
  ...
  it("registers three instant fields and a distinct multiple formula without changing annual concepts", () => {
    expect(PERSONAL_FINANCIAL_SCREEN_METRICS.slice(19, 22)).toEqual([
      "currentAssets",
      "currentLiabilities",
      "currentRatio",
    ]);
...
describe("selected revenue year-over-year growth", () => {
  ...
  it("appends one typed percentage field and retains both source roles with different filings across years", () => {
    ...
    expect(PERSONAL_FINANCIAL_SCREEN_METRICS).toHaveLength(28);
    expect(PERSONAL_FINANCIAL_SCREEN_METRICS[27]).toBe("revenueGrowth");
