...
export interface SyntheticFilingQualityMeasurementDocuments {
...
const CANDIDATE_PRODUCED_AT = "2026-01-03T00:00:00.000Z";
...
export function buildSyntheticFilingQualityMeasurementDocuments(): SyntheticFilingQualityMeasurementDocuments {
  const plan = buildPlan();
  const planBytes = canonicalSyntheticFilingQualityMeasurementDocument(plan);
  ...
  const declaredReferenceBytes =
    canonicalSyntheticFilingQualityMeasurementDocument(declaredReference);
  ...
  const candidate = {
    ...
    documentObservations: documents.map((document, index) =>
  ...
  return Object.freeze({
    candidate: canonicalSyntheticFilingQualityMeasurementDocument(candidate),
    declaredReference: declaredReferenceBytes,
    plan: planBytes,
  });
}

export function decodeSyntheticFilingQualityMeasurementDocument(
  bytes: Uint8Array,
): Record<string, unknown> {
  return JSON.parse(new TextDecoder().decode(bytes)) as Record<string, unknown>;
}

export function canonicalSyntheticFilingQualityMeasurementDocument(
  value: unknown,
): Uint8Array {
...
function buildReferenceDocument(ordinal: number): {
  ...
  return {
    ...
    facts: FILING_QUALITY_MEASUREMENT_FACT_KEYS.map((factKey, index) => ({
...
function canonicalJson(value: unknown): string {
  ...
  if (
    ...
    throw new TypeError("Synthetic quality document is not canonical data.");
  }
