...
export type DeclaredValidatorAInvalidCode =
  "normalized_payload_invalid" | "report_invalid" | "validator_binding_invalid";

export type DeclaredValidatorAResult =
  | {
  ...
  | {
      readonly code: DeclaredValidatorAInvalidCode;
...
class ValidationFailure extends Error {
  public constructor(public readonly code: DeclaredValidatorAInvalidCode) {
    super("Declared validator A rejected an envelope.");
  }
}

export function validateDeclaredValidatorAEnvelope(
  bytes: Uint8Array,
): DeclaredValidatorAResult {
  try {
...
function exactRecord<const TKeys extends readonly string[]>(
  value: unknown,
  expectedKeys: TKeys,
  code: DeclaredValidatorAInvalidCode,
...
function invalid(code: DeclaredValidatorAInvalidCode): never {
