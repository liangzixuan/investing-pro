...
offline-review verdict. One exact recovery child, its full green gates, and
...
| Frozen source transition        | One exact linear three-commit recovery chain from the baseline through both failed revisions is frozen by ancestry, path, status, and content expectation with no unreviewed widening                                 | Pending                   |
...
revision `061944f8f770e8a08b2a38d1e2fedf8b8e2de348`, with that revision itself
...
Before promotion, one exact linear three-commit recovery chain from baseline
...
`061944f8f770e8a08b2a38d1e2fedf8b8e2de348`, to one exact recovery child must
be frozen by all three exact parent lines plus the complete cumulative path and
...
Failed runs `32910394736` and `32912204603`, each attempt 1, are explicitly
non-evidence. Neither missing artifact may be substituted for the required
successful artifact; completing live execution phases in the second run does
not convert its later evidence-assembly failure into passing evidence.
...
failed precursor, failed corrective revision, and recovery revision, cumulative
