...
transport v14 and formula-set1.7. Both API and browser continue to admit the exact
version1 payload. Version2 requires every view to have a `display` field:

- `null` retains a legacy criteria-only definition.
- `{ visibleMetrics: [...] }` records one to twenty-eight distinct existing metric
...
The reported interest excluding capitalized interest and income taxes net of refunds,
common-dividend and common-stock repurchase payments,
...
only an explicit column edit and save adds new payment fields. Literal older
twenty-six-column and two-column Common stock payments layouts also stay exact.
Interest and income-tax payments are available through individual column choices
and All metrics; no existing preset gains them automatically.
