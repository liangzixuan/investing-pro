...
transport v11 and formula-set1.7. Both API and browser continue to admit the exact
version1 payload. Version2 requires every view to have a `display` field:

- `null` retains a legacy criteria-only definition.
- `{ visibleMetrics: [...] }` records one to twenty-two distinct existing metric
...
The reported total-assets, total-liabilities, cash and parent-equity fields are available for an
