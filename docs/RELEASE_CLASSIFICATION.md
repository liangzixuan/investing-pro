...
Release cases are no longer limited to a64. Canonical numeric case identifiers
and all emitted commit counts must remain exact safe integers. Registry entries
must still form the complete sequence from a14, and every historical identity,
inventory and generated byte check remains required. This removes a release-number
ceiling, not the existing descriptor, Git or generated-output size limits.
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `version`             | Exactly `1`.                                                                                                              |
| `caseNumber`          | Integer suffix in `cycle3kaN`, at least `14`, with `103 + 2 * caseNumber` a safe integer; generated writes start at `15`. |
...
`129` and `130`. Ordinary generated writes start at **a15** and have no fixed
milestone-number ceiling. The case itself and its largest emitted count
(`closureCount + 1`, or `103 + 2 * caseNumber`) must be safe integers. Installed
filenames use canonical decimal suffixes: no leading zeros, signs, exponents or
alternate extensions. Numeric ordering must still yield every case from a14.
The generator does not rewrite a14 or retrofit earlier accepted releases.

Numeric admission is not an unlimited-history claim. Each generated YAML file
must remain below 500,000 bytes and each other generated adapter below 2,000,000
bytes; descriptor and Git-read bounds also remain unchanged. Historical routing
growth may reach these practical limits before any numeric limit. A failure
requires separately reviewed maintenance, not a size-limit bypass.
...
Generator-maintenance releases retain the manual-preparation requirement. Freeze
and review the feature first; prepare the same nine closure outputs independently
from its accepted predecessor, without importing or running the changed generator
or renderer to produce them. Review the exact added routing and preservation of
all historical decisions. Then run both explicit-descriptor and installed-registry
read-only checks to confirm exact regeneration and historical agreement before
the separate closure commit. Do not disable the `--write` self-modification refusal
or use a renderer API as an alternative writer. The full isolated native gate and
applicable hosted jobs remain required for this manually prepared release.
