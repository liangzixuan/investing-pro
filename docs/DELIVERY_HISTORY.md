...
Archived September 24, 2026 from CURRENT_WORK.md. The text below preserves the
previous delivery record, including historical priorities and command examples.
It is not the active queue. Use [current work](./CURRENT_WORK.md), the
[product roadmap](./PRODUCT_ROADMAP.md) and [capability status](./CAPABILITY_STATUS.md)
for current decisions. Exact acceptance and source claims keep their original scope.
...
The table below predates the daily filing monitor and later releases. It is
retained as history. Use the capability scoreboard above for present functionality
and PRODUCT_ROADMAP.md for the current target; these old gaps are not a new queue.
...
## Historical financial delivery: screen cash generation after PP&E purchases
...
## Historical delivery order, superseded September 24

The priority table below is historical. The daily filing monitor and later
releases have shipped. The current delivery order is Markets home, connected
company pages, news/calendars and broader research, as defined in
[PRODUCT_ROADMAP.md](./PRODUCT_ROADMAP.md). Do not resume work from this old table.
...
3. During editing, run focused tests and typechecks for the actual changed
   behavior. Select them from the current feature guide and final source diff.
   The commands below are historical saved-DCF examples, not the test list for
   Markets home or a later feature:
